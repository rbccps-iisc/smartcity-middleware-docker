...
def output_ok(msg, **kwargs):
    detailed_msg = _output(msg, **kwargs)
    logging.info("%s %s" % (_ok(for_log=True), detailed_msg))
    print("%s %s" % (_ok(), msg))


def output_info(msg, **kwargs):
    detailed_msg = _output(msg, **kwargs)
    logging.error("%s %s" % (_info(for_log=True), detailed_msg))
    print("%s %s" % (_info(), msg))


def output_warning(msg, **kwargs):
    detailed_msg = _output(msg, **kwargs)
    logging.warn("%s %s" % (_warn(for_log=True), detailed_msg))
    print("%s %s" % (_warn(), msg))


def output_error(msg, **kwargs):
    detailed_msg = _output(msg, **kwargs)
    logging.error("%s %s" % (_err(for_log=True), detailed_msg))
    print("%s %s" % (_err(), msg))
